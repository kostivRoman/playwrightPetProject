...
const wheelPreland = wheelLandsArr.filter((land) => land.Type === "Land");
let i = 0;

test.describe("Wheel Land", () => {
	for (const land of wheelPreland) {
		// test.beforeEach(async () => {
		// 	const isConnected = await ping(land["Affilka Landing URL"]); // Replace with the URL you want to ping
		// 	if (!isConnected) {
